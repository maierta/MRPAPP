...
namespace psimag {
  ...
  public:
    ...
    /** matrix that does not own data (user is responsible for proper dim. */
    Matrix(size_type nRows, size_type nCols, const T* dat, size_type ldim=0) 
    ...
    */
    ...
    // ======================================================================  MSS
	  ...
	  void toJSN2(std::ostream& os, std::string title="Matrix", int width=13, bool showIndices= false,bool pythonFormat=true) const 
	  {
    ...
    template<typename VectorType> 
    VectorType& getCol(size_t col, VectorType& result) const {
  ...
  result[row] = (*this)(row,col);
    ...
    const RowSlice<const ThisType> getCol(size_t colIndex) const {
      return ColSlice<const ThisType>(*this,colIndex);
    }
  ...
  //---------------------------------------------------------------------- mss
    ...
    ThisType& operator = (const OperationClosure<ThisType, OP::TIMES, ThisType>& closure)
    {
      GEMM(closure.lhs,closure.rhs,*this);
      return *this;
    }
    
  //---------------------------------------------------------------------- mss

    ThisType& operator = (const UnaryOperationClosure< OP::INV, psimag::Matrix<T> >& closure)
    {
      if (*this != closure.operand)
  ...
  private:
	...
	if(!data) {
	  std::ostringstream msg;
	  msg << "ERROR in Matrix<T>: data array is empty\n"
	      << "new " << typeid(T).name() << "[" << m << "*" << n << "=" << n*m << "];\n";
	  throw std::runtime_error(msg.str());
	}
      }
  ...
  std::ostream& operator << (std::ostream& os, const Matrix<T>& A) {
    typedef typename Matrix<T>::size_type size_type;
    os << A.n_row() << " " << A.n_col() << " " << A.l_dim() << std::endl;
    for (size_type j=0; j<A.n_col(); j++) for (size_type i=0; i<A.n_row(); i++) {
	os << A(i,j) << std::endl;
      }
    return os;
...
//#ifdef PSIMAG_MATRIX_INVERT
  ...
  public:
	...
	throw std::logic_error(message.str());
      ...
      LAPACK::DGETRI(N,&INV(0,0),LDA,&pivots[0],&work[0],-1,info);
      blocksize = work[0];
...
//#endif
  ...
  //====================================================================== a matrix times something operation

  template<typename T, typename OtherType>
  const OperationClosure< psimag::Matrix<T>, OP::TIMES, OtherType >
  operator * (const psimag::Matrix<T>& left,
	      const OtherType&         right)
  {
    return OperationClosure< psimag::Matrix<T>, OP::TIMES, OtherType >(left,right);
  }
  
  //====================================================================== a matrix inversion operations

  template<typename T>
  const UnaryOperationClosure< OP::INV,  psimag::Matrix<T> >
  INV(const psimag::Matrix<T>& mat)
  {
    return UnaryOperationClosure< OP::INV, psimag::Matrix<T> >(mat);
  }
