...
public:
  ...
  momentumDomain(
      ...
      ConcurrencyType &concurrency, const size_t &numberOfkpoints,
      const size_t &numberOfkzpoints, const size_t &dimension)
      : param(parameters), conc(concurrency), a1(param.a1), a2(param.a2),
        a3(param.a3), shift(3, -0.5),
        // shift(3,0.0),
        dGInverse(3, 3), dim(dimension), nk(numberOfkpoints),
        nkz((dim == 3) ? numberOfkzpoints : 1),
        nktot((dim == 3) ? size_t(nk * nk * nkz) : size_t(nk * nk)), b(3, 3),
        momenta(nktot, 3),
  ...
  momentumDomain(
      ...
      ConcurrencyType &concurrency, const size_t &numberOfkpoints,
      const size_t &numberOfkzpoints, const size_t &dimension,
      const VectorType &p1, const VectorType &p2, const VectorType &p3)
      : param(parameters), conc(concurrency), a1(p1), a2(p2), a3(p3),
        shift(3, -0.5), dGInverse(3, 3), dim(dimension), nk(numberOfkpoints),
        nkz((dim == 3) ? numberOfkzpoints : 1),
        nktot((dim == 3) ? size_t(nk * nk * nkz) : size_t(nk * nk)), b(3, 3),
        momenta(nktot, 3), indexOfAdd(nktot, nktot), index2Components(nktot, 3)
  // dG(3,0),
  // GNorm(3,0)
  {
    set_primitiveVectors();
  ...
  momentumDomain(
      ...
      : param(parameters), conc(concurrency), a1(param.a1), a2(param.a2),
        a3(param.a3), shift(3, -0.5), dGInverse(3, 3), dim(dimension),
        nk(numberOfkpoints), nkz(1), nktot(nk * nk), b(3, 3), momenta(nktot, 3),
        // indexOfAdd(nktot,nktot),
        index2Components(nktot, 3)
  // dG(3,0),
  // GNorm(3,0)
  ...
  momentumDomain(
      const rpa::parameters<Field, MatrixTemplate, ConcurrencyType> &parameters,
      ConcurrencyType &concurrency, const std::string &path,
      size_t numberOfkpoints)
      : param(parameters), conc(concurrency), a1(param.a1), a2(param.a2),
        a3(param.a3), shift(3, -0.5), dGInverse(3, 3), nk(numberOfkpoints),
        nktot(nk), b(3, 3), momenta(nk, 3) {
    set_primitiveVectors();
  ...
  void set_primitiveVectors() {
    ...
    // Now setting up matrix dGInverse which is needed to map k-point onto its
    // index
    ...
    if (dim == 2) {
      for (size_t ikx = 0; ikx < nk; ++ikx) {
        for (size_t iky = 0; iky < nk; ++iky) {
          momenta(ikx + iky * nk, 0) =
              float(ikx) * 2. * param.pi_f / float(nk) - shift[0];
          momenta(ikx + iky * nk, 1) =
              float(iky) * 2. * param.pi_f / float(nk) - shift[1];
      ...
      for (size_t ikx = 0; ikx < nk; ++ikx) {
        for (size_t iky = 0; iky < nk; ++iky) {
          for (size_t ikz = 0; ikz < nkz; ++ikz) {
            momenta(ikz + iky * nkz + ikx * nkz * nk, 0) =
                float(ikx) * 2. * param.pi_f / float(nk - 1) - shift[0];
            momenta(ikz + iky * nkz + ikx * nkz * nk, 1) =
                float(iky) * 2. * param.pi_f / float(nk - 1) - shift[1];
            momenta(ikz + iky * nkz + ikx * nkz * nk, 2) =
                float(ikz) * 2. * param.pi_f / float(nkz - 1) - shift[2];
          }
        }
      }
    }
      ...
      for (size_t ikz = 0; ikz < nkz; ++ikz) {
        for (size_t iky = 0; iky < nk; ++iky) {
          for (size_t ikx = 0; ikx < nk; ++ikx) {
            ...
            for (size_t i = 0; i < 3; i++) {
              momenta(ind, i) =
                  (float(ikx) / float(nk) + shift[0]) * (*this).b(0, i) +
  ...
  void set_momenta(const Field &kxmin, const Field &kxmax, const Field &kymin,
                   const Field &kymax, const Field &kzmin, const Field &kzmax) {
  ...
  void set_momenta_Path7() { // (0,0) - (pi,pi) - (0,pi) - (pi,0) - (0,0)
    ...
    for (size_t ik = 0; ik < nks; ik++) { // (0,pi)--> (pi,0)
      momenta(ind, 0) = float(ik) / float(nks) * param.pi_f;
      momenta(ind, 1) = param.pi_f - float(ik) / float(nks) * param.pi_f;
      momenta(ind, 2) = 0.0;
      ind += 1;
  ...
  void set_momenta_Path1() {
    ...
    for (size_t ik = 0; ik < nks; ik++) { // (pi,pi) --> (2pi,0)
      momenta(ind, 0) = param.pi_f + float(ik) / float(nks) * param.pi_f;
      momenta(ind, 1) = param.pi_f - float(ik) / float(nks) * param.pi_f;
      momenta(ind, 2) = 0.0;
      ind += 1;
    }
    for (size_t ik = 0; ik < nks; ik++) { // (2pi,0) --> (pi,0)
  ...
  set_momenta_Path2() { // Gamma -> X -> M -> Gamma in units of rec. latt. vecs.
    ...
    for (size_t ik = 0; ik < nks;
         ik++) { // X -> M // 0.5*b1 --> 0.5*b1 + 0.5*b2
      momenta(ind, 0) = 0.5 * (*this).b(0, 0) +
                        float(ik) / float(nks) * 0.5 * (*this).b(1, 0);
      momenta(ind, 1) = 0.5 * (*this).b(0, 1) +
                        float(ik) / float(nks) * 0.5 * (*this).b(1, 1);
      momenta(ind, 2) = 0.5 * (*this).b(0, 2) +
                        float(ik) / float(nks) * 0.5 * (*this).b(1, 2);
      ind += 1;
    }
    for (size_t ik = 0; ik < nks; ik++) { // M -> Gamma // 0.5*b1+0.5*b2 --> 0
      momenta(ind, 0) = 0.5 * (*this).b(0, 0) + 0.5 * (*this).b(1, 0) -
                        float(ik) / float(nks) *
                            (0.5 * (*this).b(0, 0) + 0.5 * (*this).b(1, 0));
      momenta(ind, 1) = 0.5 * (*this).b(0, 1) + 0.5 * (*this).b(1, 1) -
                        float(ik) / float(nks) *
                            (0.5 * (*this).b(0, 1) + 0.5 * (*this).b(1, 1));
      momenta(ind, 2) = 0.5 * (*this).b(0, 2) + 0.5 * (*this).b(1, 2) -
                        float(ik) / float(nks) *
                            (0.5 * (*this).b(0, 2) + 0.5 * (*this).b(1, 2));
      ind += 1;
    }
  }
  ...
  void set_momenta_Path3() { // Gamma -> X -> M -> Gamma -> Z -> R -> A -> Z
    ...
    for (size_t ik = 0; ik < nks; ik++) { // Z -> R // 0.5*b3 --> 0.5*b1+0.5*b3
      momenta(ind, 0) = 0.5 * (*this).b(2, 0) +
                        float(ik) / float(nks) * 0.5 * (*this).b(0, 0);
      momenta(ind, 1) = 0.5 * (*this).b(2, 1) +
                        float(ik) / float(nks) * 0.5 * (*this).b(0, 1);
      momenta(ind, 2) = 0.5 * (*this).b(2, 2) +
                        float(ik) / float(nks) * 0.5 * (*this).b(0, 2);
      ind += 1;
    }
    for (size_t ik = 0; ik < nks;
         ik++) { // R -> A // 0.5*b1+0.5*b3 --> 0.5*b1+0.5*b2+0.5*b3
      momenta(ind, 0) = 0.5 * (*this).b(0, 0) + 0.5 * (*this).b(2, 0) +
                        float(ik) / float(nks) * 0.5 * (*this).b(1, 0);
      momenta(ind, 1) = 0.5 * (*this).b(0, 1) + 0.5 * (*this).b(2, 1) +
                        float(ik) / float(nks) * 0.5 * (*this).b(1, 1);
      momenta(ind, 2) = 0.5 * (*this).b(0, 2) + 0.5 * (*this).b(2, 2) +
                        float(ik) / float(nks) * 0.5 * (*this).b(1, 2);
      ind += 1;
    }
    for (size_t ik = 0; ik < nks; ik++) { // A -> Z
      momenta(ind, 0) = 0.5 * (*this).b(0, 0) + 0.5 * (*this).b(2, 0) +
                        0.5 * (*this).b(1, 0) -
                        float(ik) / float(nks) *
                            (0.5 * (*this).b(0, 0) + 0.5 * (*this).b(2, 0) +
                             0.5 * (*this).b(1, 0));
      momenta(ind, 1) = 0.5 * (*this).b(0, 1) + 0.5 * (*this).b(2, 1) +
                        0.5 * (*this).b(1, 1) -
                        float(ik) / float(nks) *
                            (0.5 * (*this).b(0, 1) + 0.5 * (*this).b(2, 1) +
                             0.5 * (*this).b(1, 1));
      momenta(ind, 2) = 0.5 * (*this).b(0, 2) + 0.5 * (*this).b(2, 2) +
                        0.5 * (*this).b(1, 2) -
                        float(ik) / float(nks) *
                            (0.5 * (*this).b(0, 2) + 0.5 * (*this).b(2, 2) +
                             0.5 * (*this).b(1, 2));
      ind += 1;
    }
  }
  ...
  void set_momenta_Path4() { // Gamma -> M -> K -> Gamma for triangular lattice
    ...
    for (size_t ik = 0; ik < nks; ik++) { // M -> K = (4*pi/3, 0)
      ...
      momenta(ind, 1) = -param.pi_f / sqrt(3.) +
                        float(ik) / float(nks) * param.pi_f / sqrt(3.);
      momenta(ind, 2) = 0.0;
      ind += 1;
    }
    for (size_t ik = 0; ik < nks; ik++) { // K -> Gamma = (0,0)
      momenta(ind, 0) =
          4. * param.pi_f / 3. - float(ik) / float(nks) * 4. * param.pi_f / 3.;
      momenta(ind, 1) = 0.0;
      momenta(ind, 2) = 0.0;
      ind += 1;
    }
  ...
  void set_momenta_Path4_b() { // Gamma -> (0.5,0) -> (1/3,1/3) -> Gamma in
                               // terms of rec. lattice b vectors

    if (conc.rank() == 0)
      std::cout << "Setting momentum path 4b \n";
    ...
    for (size_t ik = 0; ik < nks;
         ik++) { // (0.5,0) --> (1/3, 1/3): 0.5*b1 --> 1/3*b1+1/3*b2
      momenta(ind, 0) =
          (0.5 - float(ik) / float(nks) * (0.5 - 1. / 3.)) * (*this).b(0, 0) +
          float(ik) / float(nks) * 1. / 3. * (*this).b(1, 0);
      momenta(ind, 1) =
          (0.5 - float(ik) / float(nks) * (0.5 - 1. / 3.)) * (*this).b(0, 1) +
          float(ik) / float(nks) * 1. / 3. * (*this).b(1, 1);
      momenta(ind, 2) =
          (0.5 - float(ik) / float(nks) * (0.5 - 1. / 3.)) * (*this).b(0, 2) +
          float(ik) / float(nks) * 1. / 3. * (*this).b(1, 2);
      ind += 1;
    }
    for (size_t ik = 0; ik < nks; ik++) { // (1/3,1/3) --> Gamma
      momenta(ind, 0) = 1. / 3. * ((*this).b(0, 0) + (*this).b(1, 0)) *
                        (1. - float(ik) / float(nks));
      momenta(ind, 1) = 1. / 3. * ((*this).b(0, 1) + (*this).b(1, 1)) *
                        (1. - float(ik) / float(nks));
      momenta(ind, 2) = 1. / 3. * ((*this).b(0, 2) + (*this).b(1, 2)) *
                        (1. - float(ik) / float(nks));
      ind += 1;
    }
  }

  void set_momenta_Path5() { // Gamma -> M -> K -> Gamma for triangular lattice
                             // in extended BZ

    size_t nks(nk / 3);
    size_t ind(0);
    for (size_t ik = 0; ik < nks;
         ik++) { // Gamma = 0 -> M = (2*pi, -2*pi/sqrt(3))
    ...
    for (size_t ik = 0; ik < nks; ik++) { // M -> K = (8*pi/3, 0)
      momenta(ind, 0) =
          2. * param.pi_f + float(ik) / float(nks) * 2. * param.pi_f / 3.;
      momenta(ind, 1) = -2. * param.pi_f / sqrt(3.) +
                        float(ik) / float(nks) * 2. * param.pi_f / sqrt(3.);
      momenta(ind, 2) = 0.0;
      ind += 1;
    }
    for (size_t ik = 0; ik < nks; ik++) { // K -> Gamma = (0,0)
      momenta(ind, 0) =
          8. * param.pi_f / 3. - float(ik) / float(nks) * 8. * param.pi_f / 3.;
      momenta(ind, 1) = 0.0;
      momenta(ind, 2) = 0.0;
      ind += 1;
    }
  ...
  // 	std::vector<size_t> nki(3,0); nki[0] = (*this).nk; nki[1] = (*this).nk;
  // nki[2] = (*this).nkz; 	residue = 0.0; 	mapTo1BZ(k); 	VectorType
  // bi(3,0); 	for(size_t l=0;l<(*this).dim;l++) {
  // 		(*this).b.getRow(l,bi);
  // 		fki[l] = (scalarProd(k,bi)/scalarProd(bi,bi) - shift[l]) *
  // float(nki[l]); 		iki[l] = size_t(fki[l] + 1.0e-5);
  // residue
  // += fabs(fki[l]-float(iki[l])); 		iki[l] = iki[l] % nki[l];
  // 	}

  // 	// std::cout <<
  // "k,ik,residue="<<k[0]<<","<<k[1]<<","<<k[2]<<","<<iki[0]<<","<<iki[1]<<","<<iki[2]<<","<<fki[0]<<","<<fki[1]<<","<<fki[2]<<","<<residue<<"\n";
  // 	ik = index(iki[0],iki[1],iki[2]);
  // }

  void kToik(VectorType &k, size_t &ik, Field &residue,
             bool map1BZ = false) const {
    // std::vector<FieldType> k(kIn);
    std::vector<size_t> iki(3, 0);
    std::vector<Field> fki(3, 0);
    std::vector<size_t> nki(3, 0);
    nki[0] = (*this).nk;
    nki[1] = (*this).nk;
    nki[2] = (*this).nkz;
    residue = 0.0;
    ...
    for (size_t l = 0; l < (*this).dim; l++) {
      fki[l] = dGInverse(l, 0) * k[0] + dGInverse(l, 1) * k[1] +
               dGInverse(l, 2) * k[2] - shift[l] * nki[l];
  ...
  // 	// std::cout<<"q,index,n[0],n[1]"<<q[0]<<","<<q[1]<<","<<index<<"  , n:
  // "<<n[0]<<","<<n[1]<<"\n";

  // 	// // for(size_t l=0;l<2;l++) n[l]=size_t((qMesh.nk-1)*q[l]/param.pi_f);
  // // n[l] now contains the indices of the q-point below q
  // 	// for(size_t l=0;l<2;l++)
  // n[l]=size_t((qMesh.nk-1)*(q[l]+param.pi_f)/(2.0*param.pi_f)); // n[l] now
  // contains the indices of the q-point below q
  ...
  // 	// Now interpolate, see
  // <http://en.wikipedia.org/wiki/Trilinear_interpolation> 	VectorType
  // dis(2,0);
  //     dis[0] =
  //     (q[0]-(*this).momenta(ind00,0))/((*this).momenta(ind11,0)-(*this).momenta(ind00,0));
  //     dis[1] =
  //     (q[1]-(*this).momenta(ind00,1))/((*this).momenta(ind11,1)-(*this).momenta(ind00,1));
  ...
  // 	std::cout << "q,q00,q10,q01,q11:  "<<q[0]<<","<<q[1]<<" ,
  // "<<(*this).momenta(ind00,0)<<","<<(*this).momenta(ind00,1)<<
  // " ,
  ...
  void getSurroundingIndices(VectorType &q, size_t &ind00, size_t &ind10,
                             size_t &ind01, size_t &ind11) const {

    // Find indices of vertices of cube surrounding q
    size_t index(0);
    FieldType residue(0.0);
  ...
  void getSurroundingIndices(VectorType &q, size_t &ind000, size_t &ind001,
                             size_t &ind010, size_t &ind100, size_t &ind011,
                             size_t &ind101, size_t &ind110,
                             size_t &ind111) const {
  ...
  size_t calcIndexOfAdd(const size_t i1, const size_t i2) {
    ...
    if (dim == 2)
      ...
      index = m[0] + m[1] * nk * nk + m[2] * nk;
    return index;
  }

  // size_t calcIndexOfAdd_Old(const size_t i1, const size_t i2) {
  // 	// find index of q = k[i1]+k[i2]
  // 	// Note: We assume that reciprocal lattice vectors are given by  b1, b2
  // and b3 	Field eps(1.0e-5); 	VectorType q(3);
  // 	// std::cout << "q=" << q ;
  ...
  // 		if (vProj < -eps)        for (size_t j=0; j<dim; j++) v[j] +=
  // b(i,j); 		else if (vProj > 1-eps) for (size_t j=0; j<dim; j++)
  // v[j]
  // -= b(i,j);
  // 	}
