...
public:
  ...
  ferminator(
      ...
      : param(parameters), conc(concurrency), Pi(param.pi_f), Case_(param.Case),
        nkPerSheet(param.nkPerSheet), nSheets(0), nTotal(0), kFx(0, 0),
        kFy(0, 0), kFz(0, 0), kFtoBand(0, 0), deltakF(0, 0), vkF(0, 0),
        gammaB1GkF(0, 0), FSCenters(0), FSBand(0), bands(param, model, conc),
        calcOW_(calcOW), weights(0), weights2(0)
  ...
  void setFSValues() {
    ...
    if (Case_ == "Kagome_2") {
      ...
      // 4-orbital model: 4 FS sheets total, 1 around (pi,0), one around (0,pi),
      // 2 at Gamma
      nSheets = 4;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      for (size_t iSheet = 0; iSheet < nSheets; iSheet++) {
        for (size_t ikz = 0; ikz < param.FSnkz; ikz++) {
          FieldType kz(float(ikz) * 4. * param.pi_f / float(param.FSnkz) -
                       2. * param.pi_f);
      ...
      for (size_t iSheet = 0; iSheet < nSheets; iSheet++) {
        for (size_t ikz = 0; ikz < param.FSnkz; ikz++) {
          if (conc.rank() == 0)
            std::cout << "iSheet,ikz=" << iSheet << "," << ikz << "\n";
          FieldType kz(float(ikz) * 2. * param.pi_f / float(param.FSnkz) -
                       1. * param.pi_f);
          calcKF(nkSearch, iSheet, kz, 3);
        }
      }
    } else if (Case_ == "KFeSe_10Orbit_overdoped_3D") {
      // 5 FS sheets total, two around (pi,pi), two around (-pi,pi), one (Z)
      // around (0,0,2pi) Note that the Z pocket is closed along kz
      ...
      // 6 FS sheets total, two around (pi,pi), two around (-pi,pi), two around
      // (0,0,0) Note: One of the hole-pockets around Gamma may be closed along
      // kz
    ...
    } else if (Case_ == "BaFeAs_10Orbit_3D_7Sheets" ||
               Case_ == "1111_10orbit") {
      // 7 FS sheets total, two around (pi,pi), two around (-pi,pi), 3 around
      // (0,0,0) Note: One of the hole-pockets around Gamma may be closed along
      // kz
      ...
      for (size_t iSheet = 0; iSheet < nSheets; iSheet++) {
        ...
        for (size_t ikz = 0; ikz < param.FSnkz; ikz++) {
          if (param.FSnkz > 1)
            kz = float(ikz) * 4. * param.pi_f / float(param.FSnkz) -
                 2. * param.pi_f;
          calcKF(nkSearch, iSheet, kz, 3);
        }
      }

    } else if (Case_ == "10Orbit_3D_7Sheets_00_10_01" ||
               Case_ == "1111_10orbit") {
      // 7 FS sheets total, two around (pi,pi), two around (-pi,pi), 3 around
      // (0,0,0) Note: One of the hole-pockets around Gamma may be closed along
      // kz
      ...
      for (size_t iSheet = 0; iSheet < nSheets; iSheet++) {
        ...
        for (size_t ikz = 0; ikz < param.FSnkz; ikz++) {
          if (param.FSnkz > 1)
            kz = float(ikz) * 2. * param.pi_f / float(param.FSnkz) - param.pi_f;
          calcKF(nkSearch, iSheet, kz, 3);
        }
      }

    } else if (Case_ == "5Orbit_3D_6Sheets_00_11_10_01" ||
               Case_ == "1111_10orbit") {
      // 7 FS sheets total, two around (pi,pi), two around (-pi,pi), 3 around
      // (0,0,0) Note: One of the hole-pockets around Gamma may be closed along
      // kz
      ...
      // 5 FS sheets total, two around (0,0), one each around (pi,0) and (0,pi),
      // 1 around (pi,pi)
      ...
      // 1 FS sheets total with two different kz, one around (pi,pi,0), one
      // around (pi,pi,pi)
      ...
      for (size_t iSheet = 0; iSheet < nSheets; iSheet++) {
        ...
        FieldType kz(param.kz2D);
        if (conc.rank() == 0)
          std::cout << "FS for kz = " << kz << "\n";
        calcKF(nkSearch, iSheet, kz, 3);
      }
    } else if (Case_ == "KFeAs_10Orbit_2D") {
      // 3 FS sheets total around (0,0), neglecting tiny electron pockets near
      // (pi,pi)
      nSheets = 3;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      // 1-band Hubbard bilayer with perp. running Stripess; 8 bands total. 24
      // FS sheets total.
      nSheets = 24;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      size_t nkSearch(256);
      for (size_t iSheet = 0; iSheet < nSheets; iSheet++)
        calcKF(nkSearch, iSheet, 0.0, 2);

    } else if (Case_ == "bilayer") {
      // 1-band Hubbard bilayer; 2 bands total. 2 FS sheets. Here we assume that
      // they are both closed around (pi,pi)
      nSheets = 2;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      // 1-band Hubbard bilayer; 2 bands total. 2 FS sheets, both closed around
      // (pi,pi)
      nSheets = 2;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      // beyond overdoped -->
      // 1-band Hubbard bilayer; 2 bands total. 2 FS sheets, 1 closed around
      // (pi,pi) 1 closed around Gamma
      nSheets = 2;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      // 2 bands, 4 FS sheets, 1 around Gamma, 1 around (pi,pi), 1 around X, 1
      // around Y
      nSheets = 4;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      // 2D 1-band Hubbard; 1 band, 1 FS sheet. Here we assume that it is closed
      // around (pi,pi)
      nSheets = 1;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();
      ...
      // 2D Checkerboard Hubbard; 4 bands, 2 FS sheet. Here we assume that it is
      // closed around (pi,pi)
      nSheets = 8;
      nTotal = nSheets * nkPerSheet;
      resizeContainers();

      FSCenters[0][0] = 0.5 * Pi;
      FSCenters[0][1] = 0;
      FSBand[0] = 1;
      FSCenters[1][0] = 0;
      FSCenters[1][1] = 0.5 * Pi;
      FSBand[1] = 1;
      FSCenters[2][0] = -0.5 * Pi;
      FSCenters[2][1] = 0;
      FSBand[2] = 1;
      FSCenters[3][0] = 0;
      FSCenters[3][1] = -0.5 * Pi;
      FSBand[3] = 1;
      FSCenters[4][0] = -0.5 * Pi;
      FSCenters[4][1] = Pi;
      FSBand[4] = 1;
      FSCenters[5][0] = 0.5 * Pi;
      FSCenters[5][1] = Pi;
      FSBand[5] = 1;
      FSCenters[6][0] = Pi;
      FSCenters[6][1] = 0.5 * Pi;
      FSBand[6] = 1;
      FSCenters[7][0] = Pi;
      FSCenters[7][1] = -0.5 * Pi;
      FSBand[7] = 1;
      ...
      for (size_t iSheet = 0; iSheet < 2; iSheet++)
        calcKF(nkSearch, iSheet, 0.0, 2);
      for (size_t iSheet = 2; iSheet < 3; iSheet++)
        calcKFOpen2D(nkSearch, iSheet, 1);
      ...
      size_t nkSearch(100000);
      for (size_t iSheet = 0; iSheet < 1; iSheet++)
        calcKFOpen2D(nkSearch, iSheet, 1);
      calcDeltaKFOpen2D();
      mirrorSheet();
      for (size_t iSheet = 1; iSheet < 2; iSheet++)
        calcKFOpen2D(nkSearch, iSheet, 1);
      calcDeltaKFOpen2D();
      mirrorSheet();
      ...
      size_t nkSearch(1000);
      for (size_t iSheet = 0; iSheet < nSheets; iSheet++)
        calcKF(nkSearch, iSheet, Pi, 2);
    }

    if (conc.rank() == 0)
      std::cout << "Number of kF points: " << nTotal << "\n";

    if (kFx.size() != nTotal) {
      if (conc.rank() == 0)
        std::cout << kFx.size() << " kF points instead of " << nTotal
                  << ". Perhaps closed FS?\n";
      nTotal = kFx.size();
    }
  }
  ...
  void calcKF(const size_t nkSearch, const size_t iSheet, const FieldType &kz,
    ...
    for (size_t ik = 0; ik < nkPerSheet; ik++) {
      ...
      if (ikF <= nkSearch) {
        ...
        gammaB1GkF.push_back(
            calcgammaB1GkF(kFx[ic], kFy[ic], kFz[ic], FSBand[iSheet], dim));
        if (calcOW_)
          calcWeights(ic);
      }
    }
  ...
  void calcKFOpen2D(const size_t nkSearch, const size_t iSheet,
                    const size_t scanAlongDir) {
    VectorType w(param.nOrb);
    ComplexMatrixType v(param.nOrb, param.nOrb);
    ...
    for (size_t ik = 0; ik < nkPerSheet; ++ik) {
      ...
      k[scanAlongDir == 1 ? 0 : 1] +=
          float(ik) * 2 * Pi / float(nkPerSheet - 1);
  ...
  FieldType calcDeltaKF(const FieldType &kFx, const FieldType &kFy,
                        const FieldType &kFz, const size_t iSheet, size_t dim) {
    ...
    if (dim == 2) {
      ...
      return sqrt(pow(rx, 2) + pow(ry, 2)) * 2. * Pi / float(nkPerSheet) * dkz /
             cos(Theta);
    }
  }

  void calcDeltaKF(const VectorType &kFx, const VectorType &kFy,
                   VectorType &deltakF) {
    size_t nk(kFx.size());
    for (size_t ik = 0; ik < nk - 1; ++ik) {
      deltakF[ik] =
          sqrt(pow(kFx[ik + 1] - kFx[ik], 2) + pow(kFy[ik + 1] - kFy[ik], 2));
    }
    deltakF[nk - 1] =
        sqrt(pow(kFx[0] - kFx[nk - 1], 2) + pow(kFy[0] - kFy[nk - 1], 2));
  }
  ...
  void calcDeltaKF2D() {
    ...
    for (size_t ik = 0; ik < nk - 1; ++ik) {
      ...
      if (length > 0.25 * Pi) {
        if (conc.rank() == 0)
          std::cout << "FS segment larger than pi/4. FS points not ordered? \n";
        exit(0);
      }
      deltakF[ik] = length;
    }
  ...
  FieldType calcVkF(const FieldType &kFx, const FieldType &kFy,
                    const FieldType &kFz, const size_t iband, size_t dim) {
    FieldType dk(0.0001);
    VectorType k(3, 0);
  ...
  FieldType calcgammaB1GkF(const FieldType &kFx, const FieldType &kFy,
                           const FieldType &kFz, const size_t iband,
                           size_t dim) {
    FieldType dk(0.0001);
    VectorType k(3, 0);
  ...
  FieldType calcVkFz(const FieldType &kFx, const FieldType &kFy,
                     const FieldType &kFz, const size_t iband) {
    FieldType dk(0.0001);
    VectorType k(3, 0);
    FieldType ekmz, ekpz;
  ...
  void writeKF() {
    ...
    for (size_t i = 0; i < nTotal; i++) {
      ...
      if (calcOW_) {
        // for (size_t iorb=0;iorb<param.nOrb;iorb++) os << " , " <<
        // abs(weights[i][iorb]) ;
        for (size_t iorb = 0; iorb < param.nOrb; iorb++)
          os << " , " << real(weights[i][iorb]) << " , "
             << imag(weights[i][iorb]);
        // for (size_t iorb=0;iorb<param.nOrb;iorb++) os << " , " <<
        // real(weights2[i][iorb])<< " , " << imag(weights2[i][iorb]);
      }
