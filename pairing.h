...
public:
  pairing(rpa::parameters<Field, MatrixTemplate, ConcurrencyType> &parameters,
    ...
    if (interpolateChi_ == 0) {
      ...
      if (readChi_ == 1) {
        if (conc.rank() == 0)
          readChiqTxt2(qStore, chiStore, filenameChiPP);
    ...
    if (interpolateChi_ == 0 && storeChi_ == 1 && readChi_ == 0) {
      for (size_t iq = 0; iq < nTotal; iq++)
        conc.reduce(qStore[iq]);
      for (size_t iq = 0; iq < nTotal; iq++)
        chiStore[iq].allReduce();
    }
    if (conc.rank() == 0) {
      calcZofk();
      writeGammaPPAndNorm();
  ...
  void calcGammaPP() {
    ...
    for (; !range.end(); range.next()) {
      ...
      k1[0] = FSpoints.kFx[ik1];
      k1[1] = FSpoints.kFy[ik1];
      k1[2] = FSpoints.kFz[ik1];
      k2[0] = FSpoints.kFx[ik2];
      k2[1] = FSpoints.kFy[ik2];
      k2[2] = FSpoints.kFz[ik2];
      ...
      if (GammaPPkkp < 0.0) {
        std::cout << "GammaPPkkp negative !! for q = " << q << "\n";
        exit(0);
      }
    ...
    if (conc.rank() == 0) {
      ...
      // for(size_t i=0;i<nkF;i++) for(size_t j=0;j<nkF;j++) {
      // 	gammaPP(i,j) =
      // -0.5*(gammaPPFull(i,j)+paritySign*gammaPPFull(i,negKF[j]));
      // }
    }
  }
  ...
  void calcGammaPPTerms(size_t ind, std::vector<FieldType> q, VectorType &k1,
    ...
    // 		size_t l1 = param.indexToOrb(i,1); size_t l2 =
    // param.indexToOrb(i,0); 		size_t l3 = param.indexToOrb(j,1);
    // size_t l4 = param.indexToOrb(j,0);

    // 		os << ik1 << "\t" << ik2 << "\t" << l1 << "\t" << l2 << "\t" <<
    // l3 << "\t" << l4 << "\t" << real(temps(i,j)) << "\n"; 		if
    // (ik1!=ik2) os << ik2 << "\t" << ik1 << "\t" << l1 << "\t" << l2 << "\t"
    // << l3 << "\t" << l4 << "\t" << real(temps(i,j)) << "\n";
    // 	}
    // }
      ...
      for (size_t l2 = 0; l2 < nOrb; l2++) {
        for (size_t l3 = 0; l3 < nOrb; l3++)
          for (size_t l4 = 0; l4 < nOrb; l4++) {
            ...
            // if (real(gammaOrb(ind1, ind2)) < 0.0) {
            // std::cout << l1 << " , " << l2 << " , " << l3 << " , " << l4 <<
            // ": " << gammaOrb(ind1, ind2) << "\n";
            // }

            // std::cout << "l1,l2,l3,l4,band1,band2,aks: " <<
            // l1<<","<<l2<<","<<l3<<","<<l4<<","<<
            // band1<<","<<band2<<","<<ak1(l2,band1)<<" , "<<ak1m(l3,band1)<<" ,
            ...
            if (storeGammaOrb_) {
              ...
              if (ik1 != ik2)
                os << ik2 << "\t" << ik1 << "\t" << l1 << "\t" << l2 << "\t"
                   << l3 << "\t" << l4 << "\t" << real(gammaOrb(ind1, ind2))
                   << "\t" << real(c1 * gammaOrb(ind1, ind2)) << "\n";
            }

            // c2 += conj(c1)*gammaOrb(ind1,ind2);

            // if (ind1==8 && ind2==4) std::cout << "c1,k1,k2,a...a" <<
            // c1<<ak2(l2,band2) <<  ak2m(l3,band2)
  ...
  void writeGammaPPAndNorm() {
    ...
    // First in a format suitable for reading into R using the script in
    // gammapp_diag.R
    size_t nkF(FSpoints.nTotal);
